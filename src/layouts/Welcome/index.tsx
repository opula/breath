...
import {Platform, useWindowDimensions, View, Text, TouchableOpacity} from 'react-native';
import {
  OrientationLocker,
  PORTRAIT,
...
export const Welcome = ({navigation}: Props) => {
  ...
  return (
    <>
      ...
      <View
        style={tw`bg-black opacity-85 absolute inset-0`}
      />
        ...
        <SafeAreaView style={{flex: 1}}>
          <Carousel
            ...
            renderItem={({item, index}) => (
              <View
                style={tw`flex-1 px-8 items-center justify-center`}>
                <View style={tw`pt-12 mt-12`}>
                  <Text style={tw`text-2xl font-lusitana text-white text-center`}>
                    {item.title}
                  </Text>
                  <Text style={tw`text-base font-lusitana text-white text-center mt-4`}>
                    {item.message}
                  </Text>
                </View>
              </View>
      ...
      <View style={[tw`absolute left-0 right-0`, {bottom: bottom + 80}]}>
      ...
      <AnimatePresence exitBeforeEnter>
        {isLastCard ? (
          <MotiView
            style={[tw`absolute left-0 right-0 items-center justify-center`, {bottom: bottom + 16}]}
            key={'start'}
            from={{opacity: 0}}
            animate={{opacity: 1}}
            exit={{opacity: 0}}
            ...
            <TouchableOpacity
              ...
              <Text style={tw`text-base font-lusitana font-bold text-white`}>Get started</Text>
            </TouchableOpacity>
          </MotiView>
        ) : (
          <MotiView
            ...
            <TouchableOpacity
              ...
              <Text style={tw`text-base font-lusitana font-bold text-white`}>Next</Text>
            </TouchableOpacity>
          </MotiView>
        )}
      </AnimatePresence>
