...
export const source = Skia.RuntimeEffect.Make(`
  ...
  vec4 main(vec2 pos) {
    ...
    // Use the distance to generate Perlin noise values
    vec2 st = vec2(dist * 2.0, rad) - t;
    // float n = noise(st - vec2(t));
    float n = sin(st.x * 20.) - .8;
    n += noise(st - vec2(t)) * 1.5;

    // vec3 col = vec3(n); 
    vec3 col = mix(vec3(0.), palette(t*.5 + rad*.5), n);
    col *= 4. * rad;
